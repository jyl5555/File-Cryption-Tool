...
theme_1 = Label(opition,text="选择主题：").grid(row=0,column=0,padx=1,pady=1)
theme_2 = Combobox(master=opition,
                 text=style.theme.name,
                 values=theme_name)
theme_2.config(state="readonly")
theme_2.grid(row=0,column=1,columnspan=1,padx=1,pady=1)
...
delsource.grid(row=0,column=2,padx=1,pady=1)

name = Label(filemain,text='加密或解密的文件(可拖入):').grid(row=0,column=0,padx=1,pady=1)
name_2 = Entry(filemain,font=font)
name_2.grid(row=0,column=1,padx=1,pady=1)

password_1 = Label(filemain,text='加密或解密的密码:').grid(row=1,column=0,padx=1,pady=1)
password_2 = Entry(filemain,font=font,show="*")
password_2.grid(row=1,column=1,padx=1,pady=1)
data_1 = Label(filemain,text='保存路径(可不填):').grid(row=2,column=0,padx=1,pady=1)
data_2 = Entry(filemain,font=font)
data_2.grid(row=2,column=1,padx=1,pady=1)
...
\n\
添加文件：\n\
在"文件处理"选项卡中，可以直接拖入，或者\n\
点击"要加密或解密的文件"旁的"浏览"按钮进行\n\
浏览，也可直接输入。\n\
生成密码：\n\
点击"要加密或解密的密码"旁的"生成密钥"即可。\n\
显示密码：\n\
点击"功能"菜单里的"显示密码"即可。\n\
切换主题：\n\
"选项"选项卡中的"选择主题"菜单。\n\
...
notebook.add(filemain,text="文件处理")
notebook.add(opition,text="选项")
notebook.pack()
name_3 = Button(filemain,text='浏览...',command=openfile,bootstyle=OUTLINE).grid(row=0,column=2,padx=1,pady=1)
data_3 = Button(filemain,text='浏览...',command=savefile,bootstyle=OUTLINE).grid(row=2,column=2,padx=1,pady=1)
password_3 = Button(filemain,text='生成密钥',command=keys,bootstyle=OUTLINE).grid(row=1,column=2,padx=1,pady=1)
button = Button(filemain,text='加密或解密',command=main,bootstyle=OUTLINE).grid(row=3,column=1,padx=1,pady=1)
...
ab.add_command(label="帮助...",command=hel)
ab.add_separator()
